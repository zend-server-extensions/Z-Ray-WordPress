...
class WordPress
{
    ...
    public function initProfiler($type = 'plugins')
    {
        ...
        try {
            ...
            foreach ($objects as $filename => $object) {
                $this->zre->traceFile($filename, function () {
                }, function ($context, &$storage) use ($func, $filename) {
    ...
    {
        $this->zre->traceFunction('add_action', function () {
        }, function ($context) {
            $this->registerHook($context, 'add_action');
        });
        $this->zre->traceFunction('add_filter', function () {
        }, function ($context) {
